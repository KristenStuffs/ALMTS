...
public class RedLivingSpongeGeneration extends Feature<NoneFeatureConfiguration> {
   public RedLivingSpongeGeneration(Codec<NoneFeatureConfiguration> p_66219_) {
      super(p_66219_);
   }
   ...
   public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> p_159956_) {
      ...
      if (worldgenlevel.getBlockState(blockpos1).is(Blocks.WATER)) {
         BlockState blockstate = ModBlocks.RED_LIVING_SPONGE.get().defaultBlockState();
         BlockState blockstate1 = ModBlocks.RED_LIVING_SPONGE_PLANT.get().defaultBlockState();
         ...
         for(int l = 0; l <= k; ++l) {
            if (worldgenlevel.getBlockState(blockpos1).is(Blocks.WATER) && worldgenlevel.getBlockState(blockpos1.above()).is(Blocks.WATER) && blockstate1.canSurvive(worldgenlevel, blockpos1)) {
               ...
               if (blockstate.canSurvive(worldgenlevel, blockpos2) && !worldgenlevel.getBlockState(blockpos2.below()).is(ModBlocks.RED_LIVING_SPONGE.get())) {
